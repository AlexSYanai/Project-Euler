...
namespace challengeeleven {
  ...
  void CheckProduct(const int n) {
    if (n > high) { high = n; }
  }

  void Horizontal(const int row, const int col) {
    CheckProduct(grid[row][col] * grid[row][col+1] * grid[row][col+2] * grid[row][col+3]);
  };

  void Vertical(const int row, const int col) {
    CheckProduct(grid[row][col] * grid[row][col-1] * grid[row][col-2] * grid[row][col-3]);
  };

  void DiagonalUp(const int row, const int col) {
    CheckProduct(grid[row][col] * grid[row-1][col+1] * grid[row-2][col+2] * grid[row-3][col+3]);
  };
...
int main() {
  challengeeleven::GridProduct();

  assert(challengeeleven::high == 70600674);
  std::cout << challengeeleven::high << std::endl;
