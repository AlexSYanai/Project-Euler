...
int CalculateHorizontal(int row, int col) {
  return v_g[row][col] * v_g[row][col + 1] * v_g[row][col + 2] * v_g[row][col + 3];
}

int CalculateVertical(int row, int col) {
  return v_g[row][col] * v_g[row][col - 1] * v_g[row][col - 2] * v_g[row][col - 3];
}

int CalculateDiagonal(int row, int col) {
  return v_g[row][col] * v_g[row - 1][col + 1] * v_g[row - 2][col + 2] * v_g[row - 3][col + 3];
}

int CheckProduct(int product, int max) {
  int final = max;
  if (product > final) {
    final = product;
  }
...
int GridProduct() {
  ...
  for (int i = 0; i < 20; ++i) {
    for (int j = 0; j < 17; ++j) {
      int a = CalculateHorizontal(i, j);
      if (a > max) {
        max = a;
      }
      int b = CalculateVertical(i, j);
      if (b > max) {
        max = b;
      }
    }
